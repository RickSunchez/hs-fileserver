...
func handleGetCommand(request *models.RequestBody) (*models.ResponseGetBody, error) {
	...
	filePath, err := storage.Get(request.FileId, request.Type)
	if err != nil {
		return &response, err
	}
...
func handleAddCommand(request *models.RequestBody) (*models.ResponseAddBody, error) {
	response := models.ResponseAddBody{
		Ok: false,
	}

	fileId, err := storage.Add(request.Filename, request.Content)
	if err != nil {
		return &response, err
	}
...
func handleDeleteCommand(request *models.RequestBody) (*models.ResponseBody, error) {
	...
	err := storage.Delete(request.FileId, request.Type)
	if err != nil {
		return &response, err
	}
